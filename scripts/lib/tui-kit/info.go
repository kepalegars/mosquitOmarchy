...
// scroll, "any key to continue" otherwise.
func (i Info) ShortcutsHint() string {
	if strings.Contains(i.viewport.View(), "\n") && i.viewport.TotalLineCount() > i.viewport.Height {
		return StyleHelp.Render("↑/↓ scroll · esc/enter to continue")
	}
	return StyleHelp.Render("any key to continue")
}
