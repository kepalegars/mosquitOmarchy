...
// "mosquitomarchy" wordmark (ANSI Shadow, the whole word on one line) plus
// the "setup" subtitle (Small figlet). On wide-enough panels the two sit
// side by side; the always-available fallback stacks the subtitle directly
// under the box. maxW <= 0 forces side by side (caller-controlled width).
func MosquitOmarchyTitle(maxW int) string {
	...
	const gap = 3
	if maxW > 0 && boxW+gap+subW > maxW {
		return box + sub
	}

	// Vertical centering of the subtitle block against the box, then
	// side-by-side on one baseline. Subtitle gaps left-justified under a
	// left-aligned box so the combined block stays rectangular.
	topPad := (len(boxLines) - len(subLines)) / 2
	if topPad < 0 {
		topPad = 0
	}
	...
	for i, l := range boxLines {
		b.WriteString(l)
		if j := i - topPad; j >= 0 && j < len(subLines) {
			b.WriteString(strings.Repeat(" ", gap))
			b.WriteString(subLines[j])
		}
		b.WriteString("\n")
	}
