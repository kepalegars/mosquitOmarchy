...
Installs **DaVinci Resolve** (Studio or free) from the official Blackmagic installer. **ANY version is accepted** — whatever Blackmagic calls it (18, 19, 20…). The **filename** placed in `scripts/apps/davinci/` selects the edition (if both are present, it prompts):
