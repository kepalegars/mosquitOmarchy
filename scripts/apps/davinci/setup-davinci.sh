...
while (( $# )); do a="$1"; case "$a" in
  ...
  --scale) SCALE="$2"; shift ;;
  -h|--help) sed -n '2,/^# Launched from a file manager/p' "$0" | head -n -1; exit 0 ;;
...
url(){ printf '\e]8;;%s\e\\%s\e]8;;\e\\' "$1" "$2"; }
...
# Detection of the edition from the zip name (Studio or free)
edition_of(){
  case "$(basename "$1")" in
    DaVinci_Resolve_Studio_*) echo studio ;;
    DaVinci_Resolve_*) echo free ;;
    *) echo "?" ;;
  esac
}
...
do_status(){
  ...
  local n=0 z
  for z in "$SCRIPT_DIR"/DaVinci_Resolve_*_Linux.zip; do
    [[ -f "$z" ]] || continue
    n=$((n+1))
...
DAV_ZIPS=()
for z in "$SCRIPT_DIR"/DaVinci_Resolve_*_Linux.zip; do [[ -f "$z" ]] && DAV_ZIPS+=("$z"); done
...
detect_opencl_pkg(){  # OpenCL runtime according to the GPU (nvidia | amd | intel | empty)
  local gpu=""
  gpu="$(lspci -k 2>/dev/null | grep -iE 'vga|3d|display' | grep -ioE 'nvidia|amd|radeon|advanced micro|intel' | sort -u | head -n1 || true)"
  case "$gpu" in
...
install_resolve(){
  ...
  tmp="$(mktemp -d "${TMPDIR:-/tmp}/davinci-install.XXXXXX")"
  install_tmp="$tmp"
  msg "Extracting the zip archive…"
  unzip -oq "$zip" -d "$tmp" || { err "Cannot extract the zip: $zip"; return 1; }
  run="$(find "$tmp" -maxdepth 2 \( -name '*.run' -o -name 'install*.sh' \) -type f | head -n1)"
  if [[ -z "$run" ]]; then
...
LAUNE
  ...
  local icon="davinci-resolve" iconfull=""
  for c in /usr/share/icons/hicolor/scalable/apps/davinci-resolve.svg \
           "$RESOLVE_DIR"/DaVinciResolve.png "$RESOLVE_DIR"/DaVinci\ Resolve.png; do
    [[ -f "$c" ]] && { iconfull="$c"; break; }
  done
  cat > "$DESKTOP" <<DESKE
[Desktop Entry]
Type=Application
...
install_ffmpeg_plugin(){
  ...
  tmp="$(mktemp -d "${TMPDIR:-/tmp}/davinci-ffmpeg.XXXXXX")"
  install_tmp="$tmp"
  unzip -oq "$src" -d "$tmp" || { err "Cannot extract: $src"; return 1; }
  bund="$(find "$tmp" -maxdepth 3 -type d -name 'ffmpeg_encoder_plugin.dvcp.bundle' | head -n1)"
  if [[ -z "$bund" ]]; then
...
install_spektrafilm(){
  ...
  tmp="$(mktemp -d "${TMPDIR:-/tmp}/davinci-spektra.XXXXXX")"
  install_tmp="$tmp"
  unzip -oq "$src" -d "$tmp" || { err "Cannot extract: $src"; return 1; }
...
main(){
  ...
  if (( ${#DAV_ZIPS[@]} == 0 )); then
    err "No DaVinci_Resolve_*_Linux.zip in $SCRIPT_DIR/."
    davinci_download_links
    exit 1
  fi
  ...
  if [[ -x "$patch_script" ]]; then
    ...
    if bash "$patch_script" --check 2>/dev/null | grep -q 'binary patched'; then
