...
There is no built-in keybinding. Add or change one through the normal keybindings menu (the mosquitOmarchy TUI):

```
mosquitomarchy → Setup → keybindings → Add a keybinding → quick function → SuperFile  (command: foot -e spf)
```
