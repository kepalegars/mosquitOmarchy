...
#      (floating window rule + Super+Alt+O via setup-keybindings.sh).
#
# Usage :
...
#   ./setup-omarchy-vm.sh --remove      # remove helpers/menus (keeps ~/Omarchy-VM + ISO cache)
#   ./setup-omarchy-vm.sh --purge       # --remove + delete ~/Omarchy-VM and the ISO cache
#
...
# BOOT_ORDER="c" in ~/Omarchy-VM/vms/<name>/.vm-config.
# =============================================================================
...
DEFAULT_VM="omarchy"
DISK_SIZE="64G"
SHARED_DIR="$HOME/Omarchy-Shared"
...
while (( $# )); do
  case "$1" in
    ...
    --create-vm) CREATE_ONLY=true ;;  # VM creation only (manager-driven): skip menu/rule/keybinding
...
show_status() {
  ...
  echo " Keybinding  : $( [[ -f $HYPR_DIR/bindings.lua ]] && grep -qF "$KEYLABEL" "$HYPR_DIR/bindings.lua" && echo present || echo absent )"
  echo " VMs         :"
  local dir found=0
  if [[ -d $VMS_DIR ]]; then
...
if [[ $REMOVE == true ]]; then
  info "Removing the Omarchy VM module (helpers / menus / keybinding)"
  ...
  if [[ -f $KEYBINDS ]]; then
    bash "$KEYBINDS" -y --remove-key "$KEYBIND" 2>/dev/null || warn "Could not remove $KEYBIND"
  fi
  ...
  ok "Helpers, menu entry, window rule and keybinding removed"
  echo ""
...
for s in setup-omarchy-vm.sh omarchy-vm-tui.sh launch-omarchy-tui.sh; do
...
set -euo pipefail
VMS_DIR="${OMARCHY_VM_ROOT:-$HOME/Omarchy-VM}/vms"
name="${1:-}"
if [[ -z $name ]]; then
...
# The Omarchy menu action and the Super+Alt+O keybinding call
...
# Super+Alt+O) finds the TUI under a PATH name without the .sh extension.
exec "$(dirname "$(readlink -f "$0")")/omarchy-vm-tui.sh" "$@"
TUI_WRAPPER_EOF
chmod +x "$BIN_DIR/omarchy-vm-tui"
bash -n "$BIN_DIR/omarchy-vm-tui"
ok "omarchy-vm-tui wrapper deployed (menu/keybinding launch)"
...
ISO_PATH="${ISO_PATH:-}"
SHARED_DIR="${SHARED_DIR:-$HOME/Omarchy-Shared}"
...
# Recommend opening fullscreen (the shortcut is detected above).
VM_TIP="For the best experience, open the Omarchy VM fullscreen${FS_KEY:+ (${FS_KEY})}."
...
$BLOCK_BEGIN
  "setup.omarchyvm": {
    ...
    "iconFont": "omarchy",
    "label": "Omarchy VM",
    "description": "Omarchy inside Omarchy (QEMU/KVM) — create, launch, manage (Super+Alt+O)",
    "aliases": ["omarchy", "vm"],
...
# Section 5 is the SETUP wiring (menu entry, window rule, keybinding). When the
...
if [[ $CREATE_ONLY == false ]]; then
  ...
  if [[ -f $KEYBINDS ]]; then
    info "Registering $KEYBIND through setup-keybindings.sh (its marker block)"
    bash "$KEYBINDS" -y --ensure "$KEYBIND" "$KEYLABEL" "omarchy-launch-or-focus-tui omarchy-vm-tui" launch \
      || warn "Could not register $KEYBIND"
  fi
  hyprctl reload >/dev/null 2>&1 || true
fi
...
if [[ $SETUP_ONLY == true ]]; then
  echo "  • Setup only -> the manager, its menu entry, keybinding and window rule are installed."
  echo "  • No ISO downloaded and no VM created (by design)."
  echo ""
  echo "  ► Continue in the Omarchy VM manager:"
  echo "      Omarchy menu → Setup → Omarchy VM   (or press SUPER + ALT + O)"
...
echo "  • Manager    -> Super+Alt+O or Setup > Omarchy VM (omarchy-vm-tui)"
