...
INSTALL_DIR="${MOSQUITO_VM_ROOT}/macos"
...
print_instructions() {
    ...
    echo "  Open it from the Omarchy menu (Setup > macOS VM)"
    echo "  Or run: $0"
    echo ""
...
INSTALL_DIR="${MOSQUITO_VM_ROOT:-$HOME/VMs}/macos"
VMS_DIR="$INSTALL_DIR/vms"
...
# The window rule for the TUI is OWNED by setup-macos-vm.sh (a marker block
# in hyprland.lua). No global keybinding is registered.
setup_keybinding() {
    print_status "No global keybinding: open the VM manager from the Omarchy menu (Setup > macOS VM)"
}
...
main() {
    ...
    echo "  - Open the VM Manager from the Omarchy menu (Setup > macOS VM)"
