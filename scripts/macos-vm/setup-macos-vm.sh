...
#      with a floating centered window rule
...
# upstream bindings.conf appends). No global keybinding is registered; the
# module only keeps the window rule and the Omarchy menu entry.
#
# Usage :
#   ./setup-macos-vm.sh            # deploys everything (idempotent, auto-detects)
#   ./setup-macos-vm.sh --remove   # removes helpers + menu/window rule (keeps ~/VMs/macos)
#
...
VMS_ROOT="${MOSQUITO_VM_ROOT}/macos"
...
# The managed-bindings primitives (kb_remove) replace the old
# setup-keybindings.sh (now part of the mosquitOmarchy TUI).
...
TUI_WRAPPER_EOF
  ...
  ok "macos-vm-tui wrapper deployed (menu/app launch)"
...
$BLOCK_BEGIN
  "setup.macosvm": {
    "icon": "󰀵",
    "label": "macOS VM",
    "description": "macOS VMs in QEMU/KVM — create, launch, delete",
...
if [[ $REMOVE == false ]]; then
  ...
  # No keybinding is registered on purpose: the manager is reached from the
  # Omarchy menu (Setup > macOS VM) or the app entry — no global shortcut.
  hypr_reload_check
else
  ...
  kb_remove "SUPER + ALT + A" || warn "Could not remove Super+Alt+A"
  ok "Menu entry and window rule removed"
fi
...
info "setup-macos-vm:"
if [[ $REMOVE == true ]]; then
  echo "  • Helpers, menu and window rule removed"
  echo "  • The VMs themselves (~/VMs/macos) are kept. Delete them manually if wanted."
else
  echo "  • Installer -> osx-kvm-installer.sh  (interactive: packages, recovery, VM)"
  echo "  • Manager  -> the 'macOS VM' menu entry (Setup > macOS VM), or the TUI"
