...
# Omarchy moved the compose to a root-owned location and only rewrites it through
# `omarchy-windows-vm`. Prefer it when present; fall back to the old per-user path
# for pre-migration installs (and for the osx-style manual setups).
OMARCHY_WINDOWS_COMPOSE="/var/lib/omarchy/windows/docker-compose.yml"
LEGACY_COMPOSE="$HOME/.config/windows/docker-compose.yml"
COMPOSE="$LEGACY_COMPOSE"
[[ -f $OMARCHY_WINDOWS_COMPOSE ]] && COMPOSE="$OMARCHY_WINDOWS_COMPOSE"
...
# Omarchy's migration/launch require $HOME/.windows and $HOME/Windows to be
# exactly mode 0700. A setgid bit or a default ACL makes `chmod 0700` leave the
# mode at 2700, which silently blocks `omarchy-windows-vm remove|launch` ("Could
# not safely migrate the VM"). Strip both so our helper never wedges it.
harden_shared_dirs(){
  local d
  for d in "$HOME/.windows" "$HOME/Windows"; do
    [[ -e $d ]] || continue
...
[[ -f $COMPOSE ]] || COMPOSE="$HOME/.config/windows/docker-compose.yml"
...
[[ -f $COMPOSE_FILE ]] || COMPOSE_FILE="$HOME/.config/windows/docker-compose.yml"
CONTAINER="omarchy-windows"
...
# Omarchy now owns the compose under /var/lib/omarchy/windows (root-only dir) and
# only rewrites it through omarchy-windows-vm; a hand `sed -i` there fails. Read-
# only actions (status/start/stop) keep working; config changes are handed back.
compose_is_omarchy() { [[ $COMPOSE_FILE == /var/lib/omarchy/windows/* ]]; }
ensure_editable() {
	if compose_is_omarchy; then
		die "The VM configuration is now managed by Omarchy (root-owned compose). Reconfigure with: omarchy-windows-vm install   (status/start/stop still work here)"
	fi
}
...
start_vm() {
	require_config
	# Omarchy-managed VM: let the official manager prepare the root-owned bind
	# anchors and start Docker; a direct `docker compose up` would bypass them.
	if compose_is_omarchy; then
		echo "Starting via the Omarchy manager (omarchy-windows-vm launch)..."
...
EOF
  ...
  # The dockur /oem volume is NOT part of Omarchy's compose: its root-owned
  # writer only emits /storage and /shared, so an injected /oem line is wiped on
  # the next install/migration and can wedge Omarchy's migration. We therefore
  # deliver the debloat payload through the shared folder ($HOME/Windows, seen in
  # the VM as \\host.lan\Data) and never touch the compose.
  if grep -q ":/oem" "$COMPOSE" 2>/dev/null && [[ -w $COMPOSE && -w $(dirname "$COMPOSE") ]]; then
    info "Removing the legacy /oem volume (Omarchy's compose no longer supports it)"
    sed -i "\|$HOME/.config/windows/oem:/oem|d" "$COMPOSE"
  fi
  ok "Debloat payload served from the shared folder (no /oem volume needed)"
...
MSG
  ...
  echo "  • OEM payload        -> shared folder (\\\\host.lan\\Data -> $HOME/Windows)"
