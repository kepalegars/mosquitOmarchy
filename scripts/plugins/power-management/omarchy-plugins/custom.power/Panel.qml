...
Panel {
  ...
  // Bar icon color: normal in balanced/performance, yellow in power-saver,
  // green in ultra-save. Ultra-save forces the power-saver profile, so the
  // green check must win over the yellow one when ultra-save is active.
  // The neutral fallback uses bar.barForeground (NOT bar.foreground): with a
  // transparent bar that is the wallpaper-adaptive contrast color that all the
  // other Omarchy icons inherit — the fixed theme color would stay light on a
  // light wallpaper (invisible battery icon).
  readonly property color batteryGlyphColor: {
    if (root.ultraSaveOn) return Qt.rgba(0.35, 0.85, 0.42, 1)
    if (root.liveProfile === "power-saver") return Qt.rgba(0.93, 0.80, 0.20, 1)
  ...
  function onAcPlugged() {
    ...
    acPowersaveProc.command = [
      ...
      "if power-helper ultrasave status | grep -q '^state\ton$'; then power-helper ultrasave off; fi"
    ]
