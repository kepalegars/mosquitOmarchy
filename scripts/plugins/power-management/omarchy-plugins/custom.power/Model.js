...
function modeLabel(device, onBattery, states) {
  ...
  var s_ = states || {}
  if (chargeThresholdActive(d, onBattery, s_)) return "Threshold"
  if (onBattery) return "On battery"
  // Plugged-in but not actively charging (capacity limit / threshold wait /
  // charger hiccup): the LABEL says it, the icon stays the bolt.
  if (!onBattery && d.state === s_.PendingCharge) return "Plugged in — waiting"
  if (!onBattery && (d.state === s_.FullyCharged || percentage >= 1)) return "Fully charged"
  if (!onBattery && d.state !== s_.Charging) return "Plugged in (not charging)"
  return "Charging"
}
