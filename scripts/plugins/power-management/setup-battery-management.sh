...
deploy_bin() {
  ...
  if [[ -z "$existing" ]]; then
    warn "$name not found (neither in $SCRIPT_DIR nor in $REAL_HOME/.local/bin) — ignored."
    return 0
  fi
  if [[ -f "$dst" ]] && cmp -s "$src" "$dst" 2>/dev/null; then
    ok "$name already in place and up to date ($BIN_DIR/$name)"
  else
    cp "$existing" "$BIN_DIR/$name"
    chmod +x "$BIN_DIR/$name"
    bash -n "$BIN_DIR/$name" || { err "Invalid syntax: $name"; return 1; }
    [[ "$existing" == "$dst" ]] && ok "$name updated from repo copy" || ok "$name deployed"
  fi
}
...
ensure_power_plugin() {
  ...
  # Replace any reference to the old name in shell.json
  sed -i "s|\"$REAL_USER\\.power\"|\"$PLUGIN_ID\"|g" "$SHELL_JSON"
  sed -i "s|\"$REAL_USER\\.power\"|\"$PLUGIN_ID\"|g" "${PLUGIN_DIR}/"*.qml 2>/dev/null || true

  omarchy plugin enable "$PLUGIN_ID" >/dev/null 2>&1 || true
  ok "Omarchy plugin $PLUGIN_ID cloned and enabled (bar widget)"
}
