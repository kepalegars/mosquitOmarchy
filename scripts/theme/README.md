...
> The `achraff` module of `mosquitomarchy-setup.sh` delegates here (the `achraf67.png` image is forced), but this script remains usable standalone to create any theme from an image in `Wallpapers/`.

## "Achraff 67" mode

The `achraf67.png` image produces the frozen theme `achraff-67`: lime `#b5e61d`/red palette, `Yaru-olive` icons, "achraff_67" unlock logo (locked text); Plymouth with the theme colors. This is the `achraff` module of `mosquitomarchy-setup.sh` (forced image). Name and logo never change.