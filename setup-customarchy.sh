#!/usr/bin/env bash
# setup-customarchy.sh — Single entry point for all mosquitOmarchy.
#
...
#
# Usage:
#   ./setup-customarchy.sh            # interactive: STATUS/UPDATE/SETUP/REMOVE/BACKUP-RESTORE menu
#   ./setup-customarchy.sh -y         # apply everything with the default choices (auto backup)
#   ./setup-customarchy.sh --update   # also re-runs the already-installed modules (idempotent)
#   ./setup-customarchy.sh --status   # status of all modules, without modifying anything
#   ./setup-customarchy.sh --include=<mod>  # re-offer a module you previously uninstalled
#   ./setup-customarchy.sh --uninstall # per-module uninstall (interactive [--purge: also the data])
#   ./setup-customarchy.sh --backup   # dated backup now, then exit
#   ./setup-customarchy.sh --backup --vst-backup=full   # idem + full VST archive (~/VST)
#   ./setup-customarchy.sh --list     # chronological list of the backups
#   ./setup-customarchy.sh --restore[=FILE]  # restore a backup (chronological choice)
#   ./setup-customarchy.sh --update-repo     # git pull the scripts from GitHub (see "Updating")
#
...
# the four v0.1.0 scripts (davinci setup, bootstrap, handbrake, keybindings).
# The main script — the whole repo — is v0.1.0 (the root VERSION file).
REPO_VERSION="$(cat "$SCRIPT_DIR/VERSION" 2>/dev/null || echo 0.1.0)"
module_version(){
  case "$1" in
    davinci|handbrake|keybindings|bootstrap) echo "0.1.0" ;;
    *) echo "1.0.0" ;;
  esac
}
...
check_repo_update(){
  if repo_update_avail; then
    ...
    if ((YES)); then
      warn "(auto) not updating without confirmation — run ./setup-customarchy.sh --update-repo to pull."
      return 0
    fi
    if ask "Apply the update now (git pull, then re-run the modules)?" n; then
      if update_repo_ff; then
        ok "Update applied — run ./setup-customarchy.sh --update to re-apply the modules in their latest form."
      fi
    else
...
st_macos_vm(){
  ...
  # Keybinding is owned by setup-keybindings.sh (its marker block) → look for the
...
st_keybindings(){
  # SUPER keybindings managed by setup-keybindings.sh (marker block in bindings.lua)
...
MODULES=(
  ...
  "macos-vm:macOS VMs in QEMU/KVM (OSX-For-Omarchy) — installer + TUI manager (Super+Alt+A) + shared folders + USB passthrough"
  "omarchy-vm:Omarchy in QEMU/KVM from the official ISO — launcher + TUI manager (Super+Alt+O) + shared folder + USB/GPU passthrough"
  ...
  "davinci:DaVinci Resolve (Studio or free) + H.264/H.265 (FFmpeg encoder / libav patch) + SpectraFilm OFX option"
  ...
  "brightness:Perceptual brightness (gamma curve, 0% = screen off)"
...
run_fixes(){ # loop over FIXES_SELECTED, log results
  ...
  for id in "${FIXES_SELECTED[@]:-}"; do
    ...
    else RESULTS+=("quick-fix/$id:fail"); fi
  done
  hr
}
...
module_state(){
  ...
  case $1 in
    ...
    keybindings) st_keybindings ;; mosquitomarchy-update) st_mosquitomarchy_update ;;
...
module_of_path(){
  case "$1" in
    ...
    scripts/setup-keybindings.sh|scripts/keybindings/*)      echo keybindings ;;
...
backup_apps_selection(){
  ...
  : > "$sel_file"
  { echo "# apps.selected — apps/tuis/webapps selection saved by setup-customarchy.sh (backup)"
...
.config/systemd/user/yabridge-autosync.path \
  ...
  # --- 3. RESTORE.md (embedded help: manual OR via ./setup-customarchy.sh --restore) ---
...
Recommended method:
    ...
    cd ~/mosquitOmarchy && ./setup-customarchy.sh --restore
...
## 5. Custom scripts
    ...
    cd ~/mosquitOmarchy && ./setup-customarchy.sh --status
...
EOF
  ...
  echo "  Dedicated folder: $BACKUP_DIR      (list: ./setup-customarchy.sh --list)"
}
...
list_backups(){
  ...
  if ((${#files[@]} == 0)); then
    warn "No backup in $BACKUP_DIR (./setup-customarchy.sh --backup to create one)."
    return 0
  fi
  ...
  echo ""
  echo "  Restore: ./setup-customarchy.sh --restore"
}
...
restore_backup(){
    ...
    if ask "Restore the configuration (~/.config, ~/.local/bin wrappers, Omarchy bar)?" y; then
      ...
      if [[ -d "$HOME/.config/omarchy/plugins/custom.power" ]] \
        ...
        bash "$SCRIPT_DIR/scripts/plugins/power-management/setup-battery-management.sh" || \
          warn "Rerun 'battery' via ./setup-customarchy.sh to recreate the plugins."
      else
  ...
  echo "  Rerun ./setup-customarchy.sh --status to see what remains to install/complete."
}
...
MENU_MOSQUITO_START="// >>> Omarchy_Custom_Scripts - mosquitOmarchy setup (managed by setup-customarchy.sh)"
MENU_MOSQUITO_END="// <<< Omarchy_Custom_Scripts - mosquitOmarchy setup (managed by setup-customarchy.sh)"
...
remove_menu_blocks(){
  [[ -f $MENU ]] || return 0
  local tmp=$(mktemp) block_re='^[[:space:]]*//[[:space:]]*(>>>|<<<)[[:space:]]*Omarchy_Custom_Scripts'
  grep -qE "$block_re" "$MENU" || { rm -f "$tmp"; return; }

  # Removes the zones between >>> ... <<< markers (multi-line JSON handled),
  # then drowns any remaining marker line (case without a closing <<<).
  awk '
    BEGIN{skip=0}
    $0 ~ />>>/ && $0 !~ /<</ {skip=1; next}
    $0 ~ /<</ {skip=0; next}
  ...
  # Removes the orphan commas/objects left by a single marker
  # (covers the custom ids even without a closing <<<).
  sed -i -E '/^[[:space:]]*"[a-z_.]+"[[:space:]]*:/{ h; }' "$tmp"
  local ids='setup\.winvm|trigger\.hardware\.keyboard-backlight|system\.ultrasave|setup\.'
  sed -i -E "/[[:space:]]*\"($ids)[^\"]*\"[[:space:]]*:/d" "$tmp"
  # re-balances commas before the closing braces
  sed -i ':a;N;$!ba;s/,\(\s*\))/\1)/g' "$tmp"
...
$MENU_MOSQUITO_START
  ...
  "install.mosquitomarchy": {
    ...
    "description": "Configure this Omarchy machine (audio stack, VMs, themes, apps) — setup-customarchy launcher",
...
# setup-customarchy.sh relative to itself (../../../), so a copy in
...
un_keybindings(){
  # SUPER keybindings managed by setup-keybindings.sh: removes the marker block
  # (same block format as the brightness module).
  info "Uninstalling the SUPER keybindings (setup-keybindings.sh block)"
...
clean_plugins(){
  ...
  jq '
    ( .. | objects | select(has("moduleList")) )
      |= ( .moduleList |= map(select(.id != "custom.power" and .id != "'"$USER"'.power")) )
    | .plugins |= map(select((.id // "") | test("^(custom|mosquito|'$USER')\\.(power|lock|indicators|confirm)$") | not))
    | .disabledPlugins |= map(select(. | test("^(custom|mosquito|'$USER')\\.(power|lock|indicators|confirm)$") | not))
...
un_shared_menu(){
  # Always called: cleans the common menu blocks + Omarchy plugin state
  remove_menu_blocks
  remove_mosquitomarchy_float_rule
  clean_plugins
}
...
run_module(){
  ...
  else RESULTS+=("$id:fail"); err "Module $id: ERROR (see messages above)"; fi
  hr
}
...
run_omarchy_vm(){
  ...
  # (Omarchy menu -> Setup -> Omarchy VM, or Super+Alt+O), not from here.
...
run_keybindings(){
  # setup-keybindings.sh is an interactive manager: with -y we only report (a
  # TUI would hang waiting for input). Run it without -y to add/remove bindings.
  if ((YES)); then
    warn "keybindings module: interactive by design — run ./setup-customarchy.sh without -y"
    bash "$SCRIPT_DIR/scripts/setup-keybindings.sh" --status || true
    return 0
  fi
  bash "$SCRIPT_DIR/scripts/setup-keybindings.sh"
}
...
final_report(){
  ...
  if ((fails == 0)); then
    ...
    err "$fails module(s) failed — rerun ./setup-customarchy.sh to resume where it gets stuck."
  fi
  echo ""
...
exec_modules(){
  ...
  for id in "${SELECTED[@]:-}"; do
    case $id in
      ...
      davinci) run_module davinci run_davinci ;;
...
CATEGORIES=(
  "apps|Apps|reaper audio ableton guitarpro davinci handbrake superfile zen"
  "tuis|TUIs|"
  "webapps|Webapps|"
  "plugins|Plugins|jamjamjam-plugin battery brightness keyboard-backlight touchpad mx-master"
  "fixes|Quick fixes|"
  "mosquito|mosquito|"
...
category_run(){ # catid -> run only the CATEGORY_SELECTED items
  ...
  case $cat in
    tuis|webapps)
      ...
      bash "$APPS_DIR/setup-apps.sh" --from-backup="$tmp" -y --only="$cat" || true
      rm -f "$tmp"
      ;;
    mosquito)
      load_excluded
...
main(){
  ...
  msg "setup-customarchy — Personal scripts for configuring Omarchy (Arch/Hyprland) oriented toward audio production (REAPER, Bitwig, Windows VST, local AI). All scripts are idempotent: safe to re-run on an already-configured machine."
  status_report
